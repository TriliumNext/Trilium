...
- Interpolate with `{{var}}`. The client turns i18next's HTML-escaping off in `initLocale()` — translated strings are rendered as text, where escaping reached the reader as `&#x2F;` — so protection sits at the sinks instead: the confirm/info dialogs sanitize a string message with DOMPurify (`SanitizedHtml`), `context_menu.ts` appends a title as a text node, and `note_autocomplete.ts` escapes the term it splices into `highlightedNotePathTitle`. Don't add `escapeHtml()` at a `t()` call site; keep the sink safe. The server catalogue still escapes, hence `{{- var}}` on its quote-carrying values. Interpolated **components** whose order can vary by language (links, note references) use `<Trans>` from `react-i18next`, not `t()`.
