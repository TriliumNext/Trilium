...
- `docs/User Guide/` — Edit via `pnpm edit-docs:edit-docs`, not manually. It is also the **single source of the in-app help**; see below
- `docs/Developer Guide/` and `docs/Release Notes/` — Safe for direct Markdown editing

### In-app help (the `_help` subtree)

The User Guide is exported **once**, as markdown. `edit-docs` derives two artifacts from that export and commits them to `apps/server/src/assets/help/`:

- `help_meta.json` — the note tree: ids, titles, types, labels, and each page's `source` (its markdown file) and `dir` (for folder notes, which have no file of their own and are linked by directory)
- `help_content.json` — each page rendered to HTML, keyed by note ID, with links rewritten to `#root/_help_<noteId>` and assets to `{{helpAssets}}/<path>`

Both are **generated** — never hand-edit them; run `pnpm edit-docs:edit-docs` and let the export rewrite them. The generators are `apps/edit-docs/src/help_meta_generator.ts` and `help_bundle_generator.ts`, both pure and unit-tested.

Help pages are **virtual notes** (`services/virtual_notes.ts`): they exist only in becca, are never persisted or synced, and are read-only via an inheritable `readOnly` label on the `_help` root. Content reaches them through `InAppHelpProvider.getHelpContent()`, and `getHelpAssetBase()` decides where the `{{helpAssets}}` placeholder points — that is how one bundle serves every platform.

**Images and attachments live only in `docs/User Guide/`** and are not committed anywhere else, so **every packaging path needs its own copy step**. There are four, and missing one fails only in that one place:

| Where | Runtime | Build |
|---|---|---|
| server | `getHelpAssetDir()` reads `docs/User Guide` when `isDev` | `apps/server/scripts/build.ts` copies it into `assets/help/` |
| desktop | same (it runs the server) | `apps/desktop/scripts/build.ts` needs its **own** copy — it copies `apps/server/src/assets`, which is not where these live |
| standalone | dev middleware in `vite.config.mts` | `copyHelpAssetsPlugin` in the same file |
| mobile | inherits the standalone dist (`webDir`) | — |

Adding a static asset route also means adding it to the **dev allowlist** in `apps/server/src/routes/assets.ts` — see the warning under Server-Side Static Assets.
...
#### A new static route needs a dev path too, and a 200 does not mean it works

In dev, Vite's middleware owns whole URL prefixes and answers anything it does not recognise with `index.html`. A missing static route therefore returns **200 with `text/html`**, not a 404 — so `curl -o /dev/null -w "%{http_code}"`, and any test that only checks the status, passes while every asset is broken. **Check `Content-Type` (or the body), never the status alone.**

Two places apply the fallback, and both need the route added explicitly:

- **server** — `apps/server/src/routes/assets.ts` mounts Vite on `/${assetUrlFragment}/` with an allowlist (`/images/`, `/doc_notes/`, `/help/`) of prefixes that fall through to the static mounts. A path not on that list is swallowed
- **standalone** — `apps/standalone/vite.config.mts`; `viteStaticCopy` targets are served in dev, but anything copied by a custom plugin needs its own `configureServer` middleware (see `copyHelpAssetsPlugin` and `pdfjsServePlugin`). A plugin marked `apply: "build"` does nothing under `standalone:start`
