...
function EditorFeatures() {
    return (
        <OptionsSection title={t("editorfeatures.title")}>
            <EditorFeature name="mathlive-enabled" optionName="mathDisableMathField" label={t("editorfeatures.mathlive_enabled")} description={t("editorfeatures.mathlive_enabled_description")} inverse />
...
function EditorFeature({ optionName, name, label, description, inverse = false }: { optionName: OptionNames, name: string, label: string, description: string, inverse?: boolean }) {
    const [ featureEnabled, setFeatureEnabled ] = useTriliumOptionBool(optionName);

    return (
        <FormCheckbox
            name={name} label={label}
            currentValue={inverse ? !featureEnabled : featureEnabled}
            onChange={value => setFeatureEnabled(inverse ? !value : value)}
            hint={description}
        />
    );
}
