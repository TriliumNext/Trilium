...
class SearchResult {
    ...
    computeScore(
        ...
        preNormalized = false,
        precomputed?: { normalizedQuery?: string; queryTrigrams?: Set<string> }
    ) {
        const note = becca.notes[this.noteId];
        const normalizedQuery = precomputed?.normalizedQuery ?? normalizeSearchText(fulltextQuery);
        const queryTrigrams = precomputed?.queryTrigrams ?? getTrigrams(normalizedQuery);
        ...
        for (const token of tokens) {
            const t = preNormalized ? token : normalizeSearchText(token);
...
export function rankSearchResults (
    ...
    for (const notePathArray of notePaths) {
        const res = new SearchResult(notePathArray);
        ...
        // 🔹 quick trigram prefilter to skip clearly irrelevant notes
        if (enableFuzzyMatching && trigramJaccard(queryTrigrams, titleTrigrams) < 0.1) continue;

        res.computeScore(fulltextQuery, normalizedTokens, enableFuzzyMatching, true, {
            normalizedQuery,
            queryTrigrams
        });
        results.push(res);
