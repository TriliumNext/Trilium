...
/**
 * Dispatches a Web Fetch `Request` through an Express application in-process and
 * returns a Web `Response`, synthesising an IncomingMessage-shaped `Readable` for
 * the request and a node-mocks-http response so the real session, CSRF,
 * body-parser, multer and error middleware all run.
 *
 * Deliberately free of any `electron` import: both the main process (behind the
 * `trilium-app://` protocol handler) and the backend utility process (behind a
 * MessagePort) dispatch through this module, and the latter has no Electron APIs.
 */
