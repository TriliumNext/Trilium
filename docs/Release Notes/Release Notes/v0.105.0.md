...
*   **Flashcards** — turn notes into study cards with FSRS scheduling, cloze deletions, note-scoped templates, Anki import/export, filtered decks and leech handling.
...
<details>
    ...
    <ul>
        <li><em>New:</em> turn any note into a flashcard while keeping normal note editing. Reviews use Again, Hard, Good and Easy ratings with server-authoritative FSRS scheduling and preview intervals.</li>
        <li><em>New:</em> cloze deletion support with <code spellcheck="false">{{cN::text}}</code> syntax and a rich-text editor toolbar button.</li>
        <li><em>New:</em> note-scoped card templates with <code spellcheck="false">{{title}}</code>, <code spellcheck="false">{{content}}</code> and <code spellcheck="false">{{ordinal}}</code> placeholders.</li>
        <li><em>New:</em> note-backed decks, filtered decks, leech review queues, manual due-date rescheduling and drag scheduling from the seven-day forecast chart.</li>
        <li><em>New:</em> import Anki <code spellcheck="false">.apkg</code> packages and export Trilium flashcards as Anki-compatible packages, including media, scheduling state and bounded review history.</li>
        <li><em>New:</em> export and import Trilium JSON scheduling data, and paste externally optimized 21-weight FSRS parameter vectors in settings.</li>
        <li><em>Technical:</em> synchronized <code spellcheck="false">flashcards</code> and <code spellcheck="false">flashcard_reviews</code> entities were added. Keep a pre-upgrade backup if you might need to downgrade to a release without flashcard tables.</li>
    </ul>
</details>
