...
- [x] FSRS optimizer results UI. Settings page accepts externally optimized 21-weight FSRS parameter vectors, validates them before saving, stores them in synced scheduler config, and offers a reset-to-defaults path. In-app optimizer progress remains deferred until a supported cross-runtime optimizer is available.
- [x] Trilium JSON export/import of scheduling state and review history (settings page).
- [x] Dedicated Anki `.apkg` import provider. It accepts one package, supports upload and desktop native in-place reads, exposes image compression, and explains imported data. Generic file import continues to recognize `.apkg` directly.
- [x] Complete Anki portability. Legacy schema-11 and current Zstandard/schema-18 packages recreate nested deck/card content, preserve tags and cloze markup, render legacy and schema-15+ templates per source card, preserve per-card deck placement, import bounded referenced front/back/cloze media, store rich imported fronts for review, retain source CSS as visible card metadata, seed Trilium scheduling from Anki card queue/type/due/interval/ease/reps/lapses, and import bounded Anki review logs as Trilium review rows. `.apkg` export writes deck/card content, referenced media, scheduling state, and review logs.
