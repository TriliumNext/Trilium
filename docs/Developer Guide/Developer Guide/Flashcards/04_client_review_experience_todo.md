...
- [x] Dedicated Anki `.apkg` import provider. It accepts one package, supports upload and desktop native in-place reads, exposes image compression, and explains imported data plus current scheduling/template limits. Generic file import continues to recognize `.apkg` directly.
- [ ] Complete Anki portability. Legacy schema-11 and current Zstandard/schema-18 packages recreate nested deck/card content, preserve tags and cloze markup, render legacy and schema-15+ templates per source card, preserve per-card deck placement, import bounded referenced front/back/cloze media, store rich imported fronts for review, retain source CSS as visible card metadata, seed Trilium scheduling from Anki card queue/type/due/interval/ease/reps/lapses, and import bounded Anki review logs as Trilium review rows. `.apkg` export remains.
