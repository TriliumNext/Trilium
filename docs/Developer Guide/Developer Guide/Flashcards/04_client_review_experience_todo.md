...
- [ ] Complete Anki portability. Legacy schema-11 and current Zstandard/schema-18 packages recreate nested deck/card content, preserve tags and cloze markup, start cards with current FSRS defaults, and import bounded referenced back/cloze media. Template-only/front-rich media and CSS, multiple Anki templates, per-card deck placement, scheduling history, and `.apkg` export remain.
