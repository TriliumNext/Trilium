...
- [ ] Complete Anki portability. Legacy schema-11 and current Zstandard/schema-18 packages recreate nested deck/card content, preserve tags and cloze markup, render legacy and schema-15+ templates per source card, preserve per-card deck placement, import bounded referenced front/back/cloze media, store rich imported fronts for review, retain source CSS as visible card metadata, and seed Trilium scheduling from Anki card queue/type/due/interval/ease/reps/lapses. Review-log history and `.apkg` export remain.
