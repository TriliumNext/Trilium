...
3.  **Property/Label-Based Sorting**: Sorting is based on the parent note's `#sorted` label:
    ...
    *   **Multi-Level Sorting**: Several of the above can be combined, separated by commas; each level is applied only where the previous ones are equal. Every level can carry its own direction with `:asc` or `:desc`, otherwise it follows `#sortDirection`. For example `#sorted="priority:desc,area,dateCreated"` sorts by priority, highest first, then by area, then by creation date. A note missing a level's label sorts after the notes that have it, whatever the direction; two notes both missing it are left to the next level.
