...
| `equiv` | Marks two notes as equivalent under the builtin type. Always self-inverse: one `~equiv` link is enough, with no definition required on the notes. By default the class expands in [Search](../../Basic%20Concepts%20and%20Features/Navigation/Search.md) and collapses on the [Note Map](../../Note%20Types/Note%20Map.md). User-defined types use a relation definition marked _Equivalence_ — see [Promoted Attributes](Promoted%20Attributes.md). |