...
NOTE: `#widget`, `#run` and `~renderNote` enable code execution, so `set_attribute` saves them with a `disabled:` prefix (`#disabled:widget`, `#disabled:run`, `~disabled:renderNote`) and they stay inactive. Set them anyway, then tell the user which note to open and that, once they have reviewed the code, they activate it by removing the `disabled:` prefix in its attribute area.
...
3. Set a `~renderNote` relation on the render note pointing to the child JSX note. It is saved as `~disabled:renderNote`, so ask the user to remove the `disabled:` prefix to activate it.
